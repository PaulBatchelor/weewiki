...
#+END_SRC
*** Group Export (Not yet implemented)
...
#+BEGIN_SRC c
{
    ...
    sqlite3_prepare_v2(db, "SELECT uuid "
                       "FROM wikizet "
                       "WHERE value LIKE ?1;", -1,
                       &stmt, NULL);
...
#+BEGIN_SRC c
{
    ...
    rc = sqlite3_prepare_v2(db,
                       "SELECT time, value FROM wikizet "
                       "WHERE UUID LIKE ?1;", -1,
                       &lookup, NULL);

    sqlite3_bind_text(lookup, 1, uuid, -1, NULL);

    rc = sqlite3_step(lookup);

    while (rc == SQLITE_ROW) {
        const char *t, *v;
        t = (const char *)sqlite3_column_text(lookup, 0);
        v = (const char *)sqlite3_column_text(lookup, 1);
        printf("%s\t%s\t%s\n",
               (t == NULL) ? "-" : t,
               uuid,
               v);
