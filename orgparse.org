                  ...
                  void *ud)
{
    ...
    for (pos = 0; pos < sz; pos++) {
        ...
        if (parse_codeblock(op, txt, sz, ud, &pos)) continue;
        if (parse_comment(txt, sz, &pos)) continue;
        else if (parse_header(op, txt, sz, ud, &pos)) continue;
...
For now, just try and check for "#+BEGIN_SRC".
                           ...
                           size_t *pos)
{
    ...
    *pos += totallen;
    ...
    if (rc) {
        fwrite(&txt[start], 1, totallen, stdout);
        printf("\n");
        printf("found command (%d): '", cmdlen);
        fwrite(cmd, 1, cmdlen, stdout);
        printf("'\n");
        printf("command str (%d): '", cmdstrlen);
        fwrite(cmdstr, 1, cmdstrlen, stdout);
        printf("'\n");
        printf("totallen: %d\n", totallen);
    }
