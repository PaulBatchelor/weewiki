    ...
    for (pos = 0; pos < sz; pos++) {
        if (mode == 0) {
            ...
            if (parse_codeblock(op, txt, sz, ud,
                ...
                continue;
            } else if (parse_name(op, txt, sz, ud, &pos)) {
                continue;
            } else {
    ...
    for (n = start; n < sz; n++) {
        if (str[n] == '\n' || n == (sz - 1)) {
            ...
            *pos += (n - start);
            ...
            break;
        } else if (parse_aux_begin(op, str, ud, sz, &n,
                                   &mark, mode, buf, len)) {
            rc = 0;
            *pos = n;
            break;
...
#+END_SRC
*** Parse Aux block (beginning)
An aux block can be started at anytime inside of a text
block.
# Blech. this has gotten messy. too many arguments
# refactor someday.
#+NAME: static_funcdefs
#+BEGIN_SRC c
static int parse_aux_begin(orgparse *op,
                           const char *str,
                           void *ud,
                           size_t sz,
                           size_t *pos,
...
#+END_SRC
** Parse Aux Block (ending)
#+NAME: static_funcdefs
#+BEGIN_SRC c
static int parse_aux_end(orgparse *op,
                         const char *str,
                         size_t sz,
                         void *ud,
                         size_t *pos,
