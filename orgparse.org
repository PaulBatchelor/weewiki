                  ...
                  void *ud)
{
    ...
    for (state.pos = 0; state.pos < sz; state.pos++) {
        if (state.mode == 0) {
            if (parse_comment(txt, sz, &state.pos)) {
                continue;
            }
            if (parse_codeblock(op, txt, sz, ud,
                ...
                continue;
            } else if (parse_name(op, txt, sz, ud, &state.pos)) {
                continue;
            } else if (parse_title(op, txt, sz, ud, &state.pos)) {
                continue;
            } else {
                parse_text(op, txt, sz, ud, &state.pos,
                           &state.mode, &blk, &state.len);
            }
        } else if (state.mode == 1) {
            parse_codeblock(op, txt, sz, ud,
                            &state.pos, &state.mode, &blk, &state.len);
        } else if (state.mode == 2) {
            if (parse_aux_doiend(txt, sz, state.pos)) {
                <<end_the_aux_block>>
            } else {
                <<increment_aux_line>>
            }
...
#+NAME: static_funcdefs
#+BEGIN_SRC c
static int parse_name(orgparse *op,
                      const char *txt,
                      size_t sz,
                      void *ud,
                      size_t *pos);
...
#+NAME: static_funcdefs
#+BEGIN_SRC c
static int parse_aux_doiend(const char *str,
                            size_t sz,
                            size_t n);
#+END_SRC
#+NAME: functions
...
#+NAME: static_funcdefs
#+BEGIN_SRC c
static int parse_title(orgparse *op,
                       const char *txt,
                       size_t sz,
                       void *ud,
                       size_t *pos);
